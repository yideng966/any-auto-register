...
class KiroRegister:
    ...
    def _extract_tokens_from_object(self, data):
        ...
        if isinstance(data, str):
            try:
                ...
                import re as _re
                for key in ("accessToken", "refreshToken", "clientId", "clientSecret", "sessionToken"):
                    m = _re.search(rf'"{key}"\s*:\s*"([^"]+)"', data)
                    if m:
                        found[key] = m.group(1)
                return found
        stack = [data]
        wanted_keys = {"accessToken", "refreshToken", "clientId", "clientSecret", "sessionToken"}
    ...
    def _solve_captcha_if_exists(self, page: Page):
        try:
            ...
            if page.locator('iframe[src*="captcha"]').count() > 0 or page.locator('.awsui-captcha').count() > 0:
    ...
    def _click_primary_button(self, page: Page):
        ...
        try:
            ...
            fallback = page.locator('button[type="submit"]:not([data-id*="awsccc"]):visible').first
    ...
    def _capture_kiro_web_tokens(self, page: Page):
        if not self._captured_tokens.get("webAccessToken") and self._captured_tokens.get("accessToken"):
            self._captured_tokens["webAccessToken"] = self._captured_tokens["accessToken"]

        if self._captured_tokens.get("webAccessToken") and self._captured_tokens.get("sessionToken"):
            return
        ...
        if not self._captured_tokens.get("webAccessToken"):
            try:
                ...
                self._captured_tokens.update(self._extract_tokens_from_object(json.loads(ls)))
                if self._captured_tokens.get("accessToken") and not self._captured_tokens.get("webAccessToken"):
                    self._captured_tokens["webAccessToken"] = self._captured_tokens["accessToken"]
            except Exception:
                pass
        ...
        if not self._captured_tokens.get("sessionToken"):
            try:
                ...
                self._captured_tokens.update(self._extract_tokens_from_object(json.loads(ss)))
            except Exception:
                pass
    ...
    def _register_desktop_client(self, region: str) -> dict:
        ...
        if not client_id or not client_secret:
            raise RuntimeError(f"桌面端 OIDC Client 注册失败: {json.dumps(response, ensure_ascii=False)[:300]}")
        return {
            "clientId": client_id,
            "clientSecret": client_secret,
            "clientSecretExpiresAt": response.get("clientSecretExpiresAt"),
            "clientIdHash": hashlib.sha1(
                json.dumps({"startUrl": KIRO_IDC_START_URL}, separators=(",", ":")).encode("utf-8")
            ).hexdigest(),
        }
    ...
    def _exchange_desktop_token(
        ...
        if not response.get("accessToken") or not response.get("refreshToken"):
            raise RuntimeError(f"桌面端 token 交换失败: {json.dumps(response, ensure_ascii=False)[:300]}")
        return response
    ...
    def _handle_desktop_auth_page(self, page: Page, email: str = "", pwd: str = ""):
        ...
        try:
            if email:
                email_input = page.locator('input[placeholder="username@example.com"], input[type="email"]').first
    ...
    def _complete_desktop_idc_flow(self, email: str = "", pwd: str = "", otp_callback=None) -> dict:
            ...
            while time.time() - started < 120:
                ...
                if otp_callback and not desktop_otp_used:
                    try:
                        ...
                        raise RuntimeError(f"桌面授权验证码处理失败: {otp_error}") from otp_error
    ...
    def fetch_desktop_tokens(self, email: str, pwd: str, otp_callback=None) -> Tuple[bool, dict]:
        page = None
        ...
        try:
            ...
            tokens = self._complete_desktop_idc_flow(email=email, pwd=pwd, otp_callback=otp_callback)
            return True, tokens
    ...
    def register(self, email: str, pwd: str = None, name: str = "Kiro User", 
                 mail_token: str = None, otp_timeout: int = 120, otp_callback=None) -> Tuple[bool, dict]:
        if not pwd:
        ...
        try:
            ...
            try:
                ...
                for field in page.locator("input").all():
                    inputs_info.append(f"id={field.get_attribute('id')} type={field.get_attribute('type')} name={field.get_attribute('name')}")
            ...
            self._type_like_human(page, 'input[placeholder="username@example.com"], input[type="email"]', email)
            self._click_primary_button(page)
            time.sleep(2)
            self._solve_captcha_if_exists(page)
            ...
            # 3. 收集邮箱验证码
            self.log("3. 等待触发 OTP...")
            otp_sel = 'input[placeholder*="6-digit" i], div[data-testid*="code-input"] input, input[name="code"], input[id*="code"], input[type="text"]:not([disabled])'
            
            otp_input = None
            try:
                otp_input = page.get_by_label("Verification code", exact=True)
                if otp_input.count() == 0:
                    otp_input = page.locator(otp_sel).first
                
                otp_input.wait_for(state="visible", timeout=15000)
            except Exception as e:
                self.log(f"未能找到 OTP 输入框，页面可能不同: {e}")
                pass
            ...
            # 4. 设定与确认密码
            self.log("4. 设定与确认密码...")
            password_ready, otp_error = self._wait_for_password_step(page, timeout_ms=15000)
            ...
            password_error = self._get_first_visible_text(page, [
            ...
            if "kiro.dev" not in page.url:
                ...
                try:
                    ...
                    pass
                try:
                    self.log(f"当前页面按钮: {page.locator('button').all_text_contents()}")
                except Exception:
                    pass
                try:
            ...
            if not self._captured_tokens.get("webAccessToken"):
                ...
                try:
                    self.log(f"localStorage: {page.evaluate('() => JSON.stringify(window.localStorage)')[:2000]}")
                except Exception:
                    pass
                try:
                    self.log(f"sessionStorage: {page.evaluate('() => JSON.stringify(window.sessionStorage)')[:2000]}")
                except Exception:
                    pass
                try:
                    cookies = [
                        ...
                        for c in self.context.cookies()
                        if "kiro.dev" in c.get("domain", "") or "aws" in c.get("domain", "")
                    ]
                    self.log(f"Cookies: {json.dumps(cookies[:20], ensure_ascii=False)}")
                except Exception:
                    pass
                if self._network_debug:
                    self.log(f"网络调试样本: {json.dumps(self._network_debug[-15:], ensure_ascii=False)}")
                try:
                    ...
                    pass
                return False, {"error": "注册看似完成，但未能提取出 OAuth Token。可能是网络拦截失败。"}
            ...
            try:
                success_debug = {
                    ...
                    "network_debug": self._network_debug[-20:],
                }
                with open("kiro_success_debug.json", "w", encoding="utf-8") as f:
                    json.dump(success_debug, f, ensure_ascii=False, indent=2)
                self.log("成功调试信息已保存为 kiro_success_debug.json")
            except Exception:
                pass
            ...
            return True, {
                ...
                "password": pwd,
                "name": name,
                "accessToken": self._captured_tokens.get("accessToken", "") or self._captured_tokens.get("webAccessToken", ""),
