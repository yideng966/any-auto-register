...
class LuckMailMailboxTests(unittest.TestCase):
    ...
    def test_mailbox_constructor_forwards_proxy_to_luckmail_client(self):
        mock_client_cls = mock.Mock()
        fake_module = SimpleNamespace(LuckMailClient=mock_client_cls)

        with mock.patch.dict(sys.modules, {"core.luckmail": fake_module}):
            LuckMailMailbox(
